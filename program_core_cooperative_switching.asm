...
load_relocate_program:                   
  ...
  .b4:
         ...
         mov dx, [es:edi + 0x14]               ; Get selector for program ds.
...
start:
  ...
  .b3:
         ...
         ; Create a TCB for user program.
         mov ecx, 0x46                           ; Specify the size of a TCB.
         call sys_routine_seg_sel:allocate_memory
         mov word [es:ecx + 0x04], 0             ; Set task state (0 - Ready). 
         call append_to_tcb_link                 ; Append created TCB to the chain.
  ...
  .do_switch:
         ...
         mov eax, [tcb_chain]                    ; Search if any ready tasks.
  .find_ready:
